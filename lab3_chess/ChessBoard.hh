...

{
    ...
    public:
        std::pair<int, int> White_EP = std::make_pair(-1,-1);
        std::pair<int, int> Black_EP = std::make_pair(-1,-1);
        ...
        bool isPieceUnderThreat_simple(int row, int column, Color color);


        void isValidScan(int n, int m);
        int isValidScan_score(int n, int m, Color turn);

        void underThreatScan(int n, int m);

        float scoreBoard();
        float score_check(int fromRow, int fromColumn, int toRow, int toColumn, float score);
