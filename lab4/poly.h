...
public:
    ...
    polynomial(Iter begin, Iter end){
        ...
        while(begin != end){
            if(begin->second != 0){
                if(begin->first > this->max_degree){
                    this->max_degree = begin->first;
                }
                if(this->hash.count(begin->first)){
                    this->hash[begin->first] = this->hash[begin->first] + begin->second;
                }
                else{
                    this->hash[begin->first] = begin->second;
                }
            }
            begin++;
        }
